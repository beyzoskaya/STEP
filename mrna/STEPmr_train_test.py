...
from model.models import STEPmr
from mrna.create_graph_and_embeddings_STEPmr import TemporalGraphDatasetMrna
from scipy.spatial.distance import cdist
from step_losses import enhanced_temporal_loss
...
def train_stgcn(dataset,val_ratio=0.2):
    ...
    model = STEPmr(args, args.blocks, args.n_vertex, gene_connections) 
...
def create_gene_temporal_plots(predictions, targets, dataset, save_dir):
    genes = list(dataset.node_map.keys())
    genes_per_page = 15  # 15 genes per page (5x3 grid)
    num_genes = len(genes)
    ...
    for page in range(num_pages):
        ...
        for i, gene in enumerate(current_genes):
            ...
            # Calculate metrics for specifc gene
...
if __name__ == "__main__":
    dataset = TemporalGraphDatasetMrna(
        csv_file='/Users/beyzakaya/Desktop/STEP/dataset/mrna/interaction_mrna.csv',
        embedding_dim=32,
        seq_len=3,
        pred_len=1
    )
